...
function maskPan(v) {
  return v.slice(0, 2) + "****" + v.slice(6);
}
function maskAadhaar(v) {
  return "XXXXXXXX" + v.slice(8);
}
// function restoreEducationRows(education = []) {
//   const tbody = document.getElementById("educationTableBody");
//   tbody.innerHTML = "";
//   education.forEach((rowData, index) => {
//     addEducationRow(index > 0);
//     const row = tbody.lastElementChild;
//     row.querySelector("[name='collegeName']").value = rowData.collegeName || "";
//     row.querySelector("[name='board']").value = rowData.board || "";
//     row.querySelector("[name='degree']").value = rowData.degree || "";
//     row.querySelector("[name='stream']").value = rowData.stream || "";
//     row.querySelector("[name='joiningYear']").value = rowData.joiningYear || "";
//     row.querySelector("[name='leavingYear']").value = rowData.leavingYear || "";
//     row.querySelector("[name='percentage']").value = rowData.percentage || "";
//   });
// }
/* ================= ERROR HELPERS ================= */
...
async function handleOnlineSync() {
  ...
  if (typeof loadOfflineSubmissions !== "function") return;
  ...
  for (const payload of pending) {
    try {
      ...
      if (res.ok && typeof removeOfflineSubmission === "function") {
...
const panRealInput = document.getElementById("pan_real");
const aadhaarInput = document.getElementById("aadhaar");
const aadhaarRealInput = document.getElementById("aadhaar_real");
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  autosaveInterval = setInterval(async () => {
    ...
    data.pan = safeVal(panRealInput);
    data.aadhaar = safeVal(aadhaarRealInput);
    await saveDraftToDB({
  ...
  if (draft) {
    draft.education = collectEducationData();
  }

  function collectEducationData() {
    return [...document.querySelectorAll(".education-row")].map(row => ({
      collegeName: row.querySelector("[name='collegeName']").value,
      ...
      joiningYear: row.querySelector("[name='joiningYear']").value,
      leavingYear: row.querySelector("[name='leavingYear']").value,
  ...
  restoreEducationRowsFromDraft();

  function restoreEducationRowsFromDraft() {
  ...
  panInput.addEventListener("input", e => {
    ...
    if (panPattern.test(v)) {
      panRealInput.value = v;       // ✅ real value stored here
      panInput.value = maskPan(v);  // ✅ masked UI
      clearError(panInput);
    } else {
      panInput.value = v;
      panRealInput.value = "";
  ...
  panInput.addEventListener("focus", () => {
    if (safeVal(panRealInput)) {
      safeSet(panInput, safeVal(panRealInput));
    }
  });
  aadhaarInput.addEventListener("input", e => {
    ...
    if (aadhaarPlain.test(v)) {
      aadhaarRealInput.value = v;
      aadhaarInput.value = maskAadhaar(v);
      clearError(aadhaarInput);
    } else {
      aadhaarInput.value = v;
      aadhaarRealInput.value = "";
    }
  });
  aadhaarInput.addEventListener("focus", () => {
    if (safeVal(aadhaarRealInput)) {
      safeSet(aadhaarInput, safeVal(aadhaarRealInput));
    }
  });
  ...
  function validateKYC(silent = false) {
    ...
    clearError(panInput);
    clearError(aadhaarInput);
    if (!panRealInput.value) {
      showError(panInput, "PAN is required", silent);
      ok = false;
    }
    if (!aadhaarRealInput.value) {
  ...
  function autoCalculateSalary() {
    ...
    safeSet(monthly, a + b + c);
    safeSet(annual, (a + b + c) * 12);
  }
  // ================= EVENT LISTENERS =================
  // Loan toggle
  ...
  (async () => {
    ...
    if (draft.fields?.pan) {
      panRealInput.value = draft.fields.pan;
      panInput.value = maskPan(draft.fields.pan);
    }
    if (draft.fields?.aadhaar) {
      aadhaarRealInput.value = draft.fields.aadhaar;
      aadhaarInput.value = maskAadhaar(draft.fields.aadhaar);
    }
    isRestoringDraft = false;
